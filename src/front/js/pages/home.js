...
export const Home = () => {
  //Esta es la versión con estados dentro del componente
      ...
      {todos && todos.length > 0 ? ( //Verifico el estado
        <ol>
          {todos.map((item, index) => {
            //Hago un map del estado y muestro los to do si existen
            return (
              <li key={index}>
                {item.label}
                <button
                  type="button" //Agrego un botón para eliminar el todo
                  onClick={() => {
